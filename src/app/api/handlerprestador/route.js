...
export async function POST(request) {
    try {
        ...
        console.log("Usuario autenticado:", user);
         ...
         const existingUser = await database.query("SELECT * FROM prestador WHERE id = ?", [user.id]);
         if (existingUser) {
        ...
        const newAfiliado = await database.query("INSERT INTO prestador (id, name, email, imageUrl, phone, password, matricula,especialidad) VALUES (?, ?, ?, ?, ?, ?, ?)",
         [id, `${firstName} ${lastName}`,
          emailAddresses[0].emailAddress,
           imageUrl, phoneNumbers[0].phoneNumber, 
           passwordEnabled, matricula,especialidad]);
